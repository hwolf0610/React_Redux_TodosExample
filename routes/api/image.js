...
// @route   GET api/image/image/:idImage
// @desc    Get current users image
// @access  Private
router.get(
  "/image/:idImagen",
  ...
  (req, res) => {
    ...
    Imagenes.findOne({ user: req.user.id, _id: req.params.idImagen })
      .populate("user", ["name", "avatar"])
      .then(image => {
        if (!image) {
          errors.noimage = "There is no image for this user";
...
router.get(
  ...
  (req, res) => {
    ...
    Imagenes.findOne({ user: req.user.id, _id: req.body.idImage })
      .populate("user", ["name", "avatar"])
      .then(image => {
        if (!image) {
          errors.noimage = "There is no image for this user";
...
router.post(
  ...
  (req, res) => {
    ...
    Imagenes.find({ user: req.user.id, _id: req.body._id }).then(imagenes => {
      // si existe, actualiza
